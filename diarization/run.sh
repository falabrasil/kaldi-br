...
plda_dir=exp/xvectors_sre_combined/
segname=segmented
...
echo "[$(date +'%F %T')] $0: download model" | lolcat
  fblocal/download_model.sh $data $model_url $nnet_dir $plda_dir || exit 1
fi
...
if [ $stage -le 1 ] ; then
  ...
  steps/make_mfcc.sh --nj 10 --cmd "$train_cmd" data/original/ \
    exp/make_mfcc/original/ mfcc/original/
  utils/fix_data_dir.sh data/original/

  echo "[$(date +'%F %T')] $0: compute vad decisions from original dataset" | lolcat
  sid/compute_vad_decision.sh --nj 10 --cmd "$train_cmd" data/original/ \
      exp/make_vad/original/ mfcc/original/

  echo "[$(date +'%F %T')] $0: creating segments file from vad decisions" | lolcat
  diarization/vad_to_segments.sh --nj 10 --cmd "$train_cmd" data/original/ \
      data/$segname/
fi
...
if [ $stage -le 2 ] ; then
  ...
  cp data/$segname/{vad.scp,segments} -v data/${segname}_cmn/
...
if [ $stage -le 3 ] ; then
  ...
  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 5G" \
      ...
      data/${segname}_cmn/ $nnet_dir/xvectors_${segname}/
fi
...
if [ $stage -le 4 ] ; then
  ...
  diarization/nnet3/xvector/score_plda.sh --cmd "$train_cmd --mem 4G" \
      --target-energy 0.9 --nj 10 $plda_dir \
      $nnet_dir/xvectors_$segname/ $nnet_dir/xvectors_$segname/plda_scores/
fi

# clustering
if [ $stage -le 5 ] ; then
  echo "[$(date +'%F %T')] $0: agglomerative hierarchical clustering" | lolcat
  if [ -f data/$segname/reco2num_spk ] ; then
    ...
    diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 10 \
        ...
        $nnet_dir/xvectors_$segname/plda_scores/ \
        $nnet_dir/xvectors_$segname/plda_scores_num_speakers/
  else
    ...
    diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 10 \
        --threshold $threshold \
        $nnet_dir/xvectors_$segname/plda_scores/ \
        $nnet_dir/xvectors_$segname/plda_scores_threshold_$threshold/
  fi
fi
