...
if $use_gpu;then
	if ! cuda-compiled; then
		echo "This script is intended to be used with GPUs but you have not compiled Kaldi with CUDA. It will use only your processor instead."
		num_threads=16
...
rm -rf RESULTS log exp mfcc data/train/spk2utt data/train/cmvn.scp data/train/feats.scp data/train/split12 data/test/spk2utt data/test/cmvn.scp data/test/feats.scp data/test/split12 data/local/lang data/lang data/local/tmp data/local/dict/lexiconp.txt
...
# 	--num-epochs $num_epochs --num-epochs-extra $num_epochs_extra --parallel-opts "$parallel_opts" --num-threads $num_threads \
...
# 	--initial-learning-rate $initial_learning_rate --final-learning-rate $final_learning_rate \
# 	--cmd "$decode_cmd" --pnorm-input-dim $pnorm_input_dim \
# 	--pnorm-output-dim $pnorm_output_dim data/train data/lang exp/tri3_500-4_ali exp/nnet4d2_tri3_500-4
...
# 	--initial-learning-rate $initial_learning_rate --final-learning-rate $final_learning_rate \
# 	--cmd "$decode_cmd" --pnorm-input-dim $pnorm_input_dim \
# 	--pnorm-output-dim $pnorm_output_dim data/train data/lang exp/tri3_500-8_ali exp/nnet4d2_tri3_500-8
...
steps/nnet2/train_pnorm_fast.sh --stage -10 \
	--minibatch-size $minibatch_size \
	--num-epochs $num_epochs --num-epochs-extra $num_epochs_extra --parallel-opts $parallel-opts --num-threads $num-threads \
	--add-layers-period 1 --num-hidden-layers $num_hidden_layers --mix-up 4000 \
	--initial-learning-rate $initial_learning_rate --final-learning-rate $final_learning_rate \
