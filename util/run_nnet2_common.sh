...
if [ $stage -le 0 ]; then
  ...
  for datadir in $train_set test; do
    ...
    steps/make_mfcc.sh --nj 2 --mfcc-config $mfcc_config \
...
if [ ! -f $extractor/final.ie ] && [ $ivector_dim -gt 0 ]; then
  if [ $stage -le 1 ]; then
    ...
    steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 2 --num-frames 200000 \
      data/${train_set} 256 exp/$gmm exp/nnet2${nnet_affix}/diag_ubm
  fi
