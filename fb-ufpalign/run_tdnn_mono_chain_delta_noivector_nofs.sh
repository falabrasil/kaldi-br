...
affix=mono_chain_delta_noivector_nofs
tree_affix=mono_chain_delta_noivector_nofs
train_stage=-10
get_egs_stage=-10
decode_iter=
...
if $decode && [ $stage -le 17 ]; then
  ...
  grep -Rw WER $dir/decode${decode_iter:+_$decode_iter}_test_tgsmall | utils/best_wer.sh
